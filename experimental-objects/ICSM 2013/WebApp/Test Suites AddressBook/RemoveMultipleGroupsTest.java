...
public class AddressBookRemoveMultipleGroupsTest {
  ...
  public void testAddressBookRemoveMultipleGroups() throws Exception {
    ...
    assertFalse(driver.findElement(By.cssSelector("BODY")).getText().matches("^[\\s\\S]*Group1[\\s\\S]*$"));
    // Warning: assertTextNotPresent may require manual changes
    assertFalse(driver.findElement(By.cssSelector("BODY")).getText().matches("^[\\s\\S]*Group2[\\s\\S]*$"));
    // Warning: assertTextNotPresent may require manual changes
    assertFalse(driver.findElement(By.cssSelector("BODY")).getText().matches("^[\\s\\S]*Group3[\\s\\S]*$"));
