...
public class Detection {
	...
	// parameter values for getting pixel-perfect match like pixel-to-pixel
	// difference
	/*
	 * private double COLOR_FACTOR = 1.0; private double FOV = 89.9; private
	 * double GAMMA = 4.2; private double LUMINANCE = 800.0; private boolean
	 * LUMINANCE_ONLY = false; private int THRESHOLD_PIXELS = 100; private int
	 * DOWNSAMPLE = 0; private int PERCENTAGE_OF_TOTAL_IMAGE_SIZE = 0;
	 */
	/*
	 * // default values from pdiff page by Hector Yee private double
	 * COLOR_FACTOR = 1.0; private double FOV = 27.0; private double GAMMA =
	 * 2.2; private double LUMINANCE = 100.0; private boolean LUMINANCE_ONLY =
	 * false; private int THRESHOLD_PIXELS = 100; private int DOWNSAMPLE = 0;
	...
	public List<Point> detectionByPixelByPixelImageDifferencing(String oracleFullPath,
			String testPageScreenshotFullPath) {
		...
		List<Point> lDifferencePixels = ip.compareImages(oracleImagePathAndName[0], oracleImagePathAndName[1],
				testImagePathAndName[0], testImagePathAndName[1]);
		return lDifferencePixels;
	}

	// perceptual image differencing
	public List<Point> detectionByPerceptualImageDifferencing(String oracleFullPath, String testPageScreenshotFullPath,
			String differenceImageFullPath) throws IOException {

		System.out.println("oracle: " + oracleFullPath);
		System.out.println("test: " + testPageScreenshotFullPath);
		...
		// (ip.getImageSize(testScreenshotPathAndName[0],
		// testScreenshotPathAndName[1]).width *
		// ip.getImageSize(testScreenshotPathAndName[0],
		// testScreenshotPathAndName[1]).height *
		// ((double)PERCENTAGE_OF_TOTAL_IMAGE_SIZE/100));
		...
		BufferedImage imgDiff = (differenceImageFullPath != null)
		...
		if (differenceImageFullPath != null) {
			...
			String formatName = (extIndex != -1) ? differenceImageFullPath.substring(extIndex + 1)
		...
		if (passed) {
			// set the number of difference pixels as zero, implying there is no
	...
	public static void main(String[] args) throws IOException {
		...
		d.detectionByPerceptualImageDifferencing(path + "oracle.png", path + "test.png", path + "diff.png");

		File directory1 = new File(path + "TestAddPhone");
		...
		File directory2 = new File(path + "TestAddPhone1");
		// get all the files from a directory
		...
		for (int i = 0; i < fList1.length; i++) {
			for (int j = 0; j < fList2.length; j++) {
				if (i == j) {
					if (fList1[i].isFile() && fList2[j].isFile()) {
						// d.detectionByPerceptualImageDifferencing(fList1[i].getPath(),
						// fList2[j].getPath(), path + "diff.png");
						System.out.println(
								ImageProcessing.compareImagesByHistogram(fList1[i].getPath(), fList2[j].getPath()));
					}
				}
			}
		}
