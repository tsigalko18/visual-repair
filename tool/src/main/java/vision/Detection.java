...
//				ip.getImageSize(testScreenshotPathAndName[0], testScreenshotPathAndName[1]).height * ((double)PERCENTAGE_OF_TOTAL_IMAGE_SIZE/100));
		...
		if (differenceImageFullPath != null)
		{
			...
			String formatName = (extIndex != -1) ? differenceImageFullPath.substring(extIndex + 1) : Settings.imageExtension;
...
//						d.detectionByPerceptualImageDifferencing(fList1[i].getPath(), fList2[j].getPath(), path + "diff.png");
						System.out.println(ImageProcessing.compareImagesByHistogram(fList1[i].getPath(), fList2[j].getPath()));
					}
				}
			}
		}     
