...
public class UtilsScreenshots {
	...
	/**
	 * Save the GUI of the current driver instance in 
	 * a file name identified by name
	 * @param d
	 * @param filename
	 * @throws AWTException 
...
//		int scale = 10;
		int scale = 4;
		getScaledSubImage(img, element, visualLocator, scale);
	...
	public static void getPreciseElementVisualCrop(WebDriver d, String filename, WebElement element, String webElementImageName) throws IOException{
		...
		getPreciseSubImage(img, element, visualLocator);
	...
	public static void saveAnnotatedScreenshot(String inFile, String templateFile, String outFile) throws IOException{
		...
		for (int i = 0; i < result_rows; i++) {
			for (int j = 0; j < result_cols; j++) {
				if (result.get(i, j)[0] >= 0.99) matches.add(new Point(i, j));
			}
		}

		if (matches.size() == 0) {
			if(Settings.verbose) System.err.println("[LOG]\tWARNING: No matches found!");
		} else if (matches.size() > 1) {
			if(Settings.verbose) System.err.println("[LOG]\tWARNING: Multiple matches!");
		}
	 ...
	 * @throws IOException
	 */	
	public static void getScaledSubImage(BufferedImage img, WebElement element, File visualLocator, int scale) throws IOException{
		...
		try {
			...
			if(Settings.verbose) System.out.println("[LOG]\ttest might have changed its state");
		}
		...
		int min_offset_x = Math.min(element.getLocation().x, img.getWidth() - rect.width
				- element.getLocation().x);
		int min_offset_y = Math.min(element.getLocation().y, img.getHeight() - rect.height
...
//		int offset = Math.max(min_offset_x, min_offset_y);
		...
		try {
			if (element.getTagName().equals("option"))
			{
				...
				subImage = img.getSubimage(elementCoordinates.x - offset, elementCoordinates.y - offset, 2
						* offset + rect.width, 2 * offset + rect.height);
			}
			else
			{
				subImage = img.getSubimage(elementCoordinates.x - offset, elementCoordinates.y - offset, 2
	 ...
	 * @throws IOException
	 */	
	public static void getPreciseSubImage(BufferedImage img, WebElement element, File visualLocator) throws IOException {
	...
	public static List<Point> returnAllMatchesForAllAlgorithms(String inFile, String templateFile) {
		...
		int methods[] = { Imgproc.TM_SQDIFF_NORMED, Imgproc.TM_SQDIFF, Imgproc.TM_CCOEFF_NORMED, 
						  Imgproc.TM_CCOEFF,  Imgproc.TM_CCORR, Imgproc.TM_CCORR_NORMED };
