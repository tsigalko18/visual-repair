...
public class Water {
	...
	public static List<HtmlElement> repairLocators(EnhancedTestCase broken, EnhancedTestCase correct,
			EnhancedException ex, HtmlDomTree oldTree, String locator, HtmlDomTree newTree) {
		...
		// HtmlElement oldNode = UtilsWater.getNodeByLocator(oldTree,
		// l.getValue());
		...
		if (!matches.isEmpty()) {
			for (HtmlElement candidateElement : matches) {
				...
				// Statement st =
				// testRepair.getStatements().get(Integer.parseInt(ex.getInvolvedLine()));
				// st.setDomLocator(new SeleniumLocator("xpath",
				// candidateElement.getXPath()));
				//
		...
		if (repairs.isEmpty()) {
			...
			for (HtmlElement similarElement : similarNodes) {
				...
				// run test case and if it passes, add the element to the the
				// repair list
				try {
					if (UtilsWater.checkRepair(testRepair)) {
