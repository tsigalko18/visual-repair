...
public class TestAnnouncement {
	...
	public void testAnnouncement() throws Exception {
		System.out.println("[TestAnnouncement]\t\tnot applicable in this version");
		...
		// assertTrue(driver.findElement(By.id("claroBody")).getText().matches("^[\\s\\S]*Use
		// 0 to display all[\\s\\S]*$"));
		...
		// for Announcement, \\(CLANN\\) are now effective on
		// server[\\s\\S]*$"));
		// driver.findElement(By.cssSelector("input[type=\"submit\"]")).click();
